	...
	function App() {
	  ...
	  y$1(function () {
	    if (frame.current == null && elapsed < duration) {
	      frame.current = requestAnimationFrame(function (now) {
	        frame.current = null;
	  ...
	  return h(d, null, h("label", null, "Elapsed time: ", h("progress", {
	    value: elapsed / duration
	  })), h("div", {
	    "class": "elapsed"
	  }, (elapsed / 1000).toFixed(1), "s"), h("label", null, "Duration:", " ", h("input", {
	    type: "range",
	    min: "1",
	    max: "20000",
	    value: duration,
