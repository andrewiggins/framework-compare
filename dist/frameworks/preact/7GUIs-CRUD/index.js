(function () {
  ...
  function createApi() {
    ...
    return {
       ...
       */
      listAll() {
        return mockFetch("/persons").then(() => copy(store));
      },
       ...
       */
      read(id) {
        return mockFetch(`/persons/${id}`).then(() => copy(getPersonById(id)));
      },
  ...
  /**
   * @typedef {{ id: number; name: string; surname: string; }} Person
   * @typedef {{ initial: boolean; create: boolean; update: boolean; delete: boolean; }} LoadingState;
  ...
  var reducer = function reducer(action) {
    return function (state) {
      ...
      switch (action.type) {
        case "INITIAL_LOAD":
          /** @type {Person} */
          var person = action.payload.length ? action.payload[0] : EMPTY_PERSON;
        ...
        case "SELECT_PERSON":
          var selectedPerson = state.persons.filter(function (person) {
  ...
  function (_Component) {
    ...
    _proto.componentDidMount = function componentDidMount() {
      ...
      api.listAll().then(function (persons) {
        _this2.setState(reducer({
          type: "INITIAL_LOAD",
          payload: persons
        }));
      });
    ...
    _proto.render = function render(props, state) {
      ...
      return h("div", {
        ...
        "class": "form-group"
      }, h("label", null, "Filter prefix:", " ", h("input", {
        type: "text",
        value: state.filter,
        onInput: this.onFilterInput
      }))), h("div", {
        "class": "form-group"
      }, h("label", null, "Select a person to edit:"), h("select", {
        size: 5,
        onChange: this.onPersonSelect
      }, filteredPersons.map(function (person) {
        return h("option", {
          key: person.id,
          value: person.id,
